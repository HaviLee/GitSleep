...
+ (UIBarButtonItem *)itemWithIcon:(NSString*)iconName showBadge:(BOOL)showbadge target:(id)obj action:(SEL)selector {
    ...
    CGRect rect = CGRectMake(button.frame.origin.x, button.frame.origin.y, imgSize.width, imgSize.height);
    [button setFrame:rect];
//    button.frame.size = CGSizeMake(imgSize.width, imgSize.height);

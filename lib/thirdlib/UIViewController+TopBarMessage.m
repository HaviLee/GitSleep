...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        self.label = [[UILabel alloc] initWithFrame:CGRectMake(0, 64, 50, 50)];
...
- (void)showTopMessage:(NSString *)warningText
{
    ...
    if (!topV) {
        topV = [[TopWarningView alloc] initWithFrame:CGRectMake(0, 64, CGRectGetWidth(self.view.bounds), 44)];
        objc_setAssociatedObject(self, &TopWarningKey, topV, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    topV.frame = CGRectMake(0, 64, CGRectGetWidth(self.view.bounds), 44);

...
- (OTCover*)initWithTableViewWithHeaderImage:(UIImage*)headerImage withOTCoverHeight:(CGFloat)height{
    ...
    self.tableView.backgroundColor = [UIColor colorWithRed:0.949f green:0.941f blue:0.945f alpha:1.00f];
    
    [self.tableView addObserver:self forKeyPath:@"contentOffset" options:NSKeyValueObservingOptionNew context:nil];
    
    self.blurImages = [[NSMutableArray alloc] init];
    [self prepareForBlurImages];
    [self.headerImageView setImage:[self.blurImages lastObject]];
    return self;
}
...
- (void)prepareForBlurImages
{
    ...
    for (NSUInteger i = 0; i < self.OTCoverHeight/20; i++) {

...
- (void)layoutSubviews {
    ...
    CGRect frame = CGRectMake(0, 0, (self.frame.size.width - 45)/3, self.frame.size.height);
    ...
    for (UIView *view in tagSubviews_) {
        ...
        if (index != 0) {
            ...
            if (tagSubviews_.count<4) {
                tempViewFrame.origin.x = prevView.frame.origin.x + prevView.frame.size.width+(self.frame.size.width -40- 3*77)/2;
            }else{
                tempViewFrame.origin.x = prevView.frame.origin.x + prevView.frame.size.width+6;
            }
        } else {
            tempViewFrame.origin.x = 20;
        }
...
- (void)reloadTagSubviews {
    ...
    for (TagObject *tagO in _tags) {
        NSString *tag = tagO.tagName;
        ...
        if (tagO.isSelect) {
            tagView.backgroundColor = [UIColor colorWithRed:0.145f green:0.431f blue:0.561f alpha:1.00f];
        }else{

...
- (id)initWithData:(NSData *)d timeout:(NSTimeInterval)t tag:(id)i;
...
- (void)notifyDidSendDataWithTag:(id)tag
{
	LogTrace();
...
- (void)notifyDidNotSendDataWithTag:(id)tag dueToError:(NSError *)error
{
	LogTrace();
...
- (void)sendData:(NSData *)data withTag:(id)tag
{
	[self sendData:data withTimeout:-1.0 tag:tag];
}

- (void)sendData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(id)tag
{
	LogTrace();
...
- (void)sendData:(NSData *)data toAddress:(NSData *)remoteAddr withTimeout:(NSTimeInterval)timeout tag:(id)tag
{
	LogTrace();

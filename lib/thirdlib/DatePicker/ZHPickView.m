...
-(void)setToolbarWithPickViewFrame{
    _toolbar.frame=CGRectMake(0, _pickeviewHeight,[UIScreen mainScreen].bounds.size.width, ZHToobarHeight);
//    自定义的
    ...
    [buttonCancel addTarget:self action:@selector(remove) forControlEvents:UIControlEventTouchUpInside];
    buttonCancel.frame = CGRectMake(0, _pickeviewHeight, [UIScreen mainScreen].bounds.size.width, (ZHToobarHeight)/2);
    ...
    [lineView makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(buttonCancel);
        make.right.equalTo(buttonCancel);
        make.top.equalTo(buttonCancel.top);
...
//    
    ...
    [buttonSure setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    [buttonSure addTarget:self action:@selector(doneClick) forControlEvents:UIControlEventTouchUpInside];
    buttonSure.frame = CGRectMake(0, _pickeviewHeight+(ZHToobarHeight)/2, [UIScreen mainScreen].bounds.size.width, (ZHToobarHeight)/2);
    [self addSubview:buttonSure];
    UIView *lineView1 = [[UIView alloc]init];
    lineView1.backgroundColor = [UIColor lightGrayColor];
    lineView1.alpha = 0.3;
    [buttonSure addSubview:lineView1];
    [lineView1 makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(buttonSure);
        make.right.equalTo(buttonSure);
        make.top.equalTo(buttonSure.top);
        make.height.equalTo(1);
    }];
}
...
-(void)remove{
    if ([self.delegate respondsToSelector:@selector(toobarDonBtnHaveClick:resultString:)]) {
        [self.delegate toobarDonBtnHaveClick:self resultString:@"取消"];
    }

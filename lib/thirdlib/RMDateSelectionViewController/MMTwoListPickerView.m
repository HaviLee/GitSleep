...
@interface MMTwoListPickerView () <UIPickerViewDelegate, UIPickerViewDataSource>
...
@implementation MMTwoListPickerView

#pragma mark - Singleton

+ (MMTwoListPickerView*)sharedView {
  static dispatch_once_t once;
  static MMTwoListPickerView *sharedView;
...
-(void)dismiss{
 [MMTwoListPickerView dismissWithCompletion:self.onDismissCompletion];
}
//
- (void)cancel
{
   [MMTwoListPickerView cancelWithCompletion:self.onDismissCompletion];
}

+(void)cancelWithCompletion:(void (^)(NSString *))completion{
    [[self sharedView] cancelPickerHidden:YES callBack:completion];
}

-(void)cancelPickerHidden: (BOOL)hidden
    ...
    [UIView animateWithDuration:0.3
                     ...
                     animations:^{
                         ...
                         if(completed && hidden){
                             [MMTwoListPickerView removePickerView];
                             callBack(@"cancel");
                         }
                     }];
...
//  UIImage * toolbarImage = options[MMtoolbarBackgroundImage];
  ...
  _pickerContainerView = [[UIView alloc] initWithFrame:CGRectMake(10.0, _pickerViewContainerView.bounds.size.height - 288.0, self.frame.size.width-20, 286.0)];
    _pickerContainerView.layer.masksToBounds = YES;
    _pickerContainerView.layer.cornerRadius = 5;
...
//  _pickerContainerView.backgroundColor = pickerViewBackgroundColor;
  ...
  //button
//    self.buttonBackView = [[UIView alloc]initWithFrame:CGRectMake(10, _pickerViewContainerView.bounds.size.height - 86.0, self.frame.size.width-20, 80)];
//    self.buttonBackView.backgroundColor = [UIColor whiteColor];
//    self.buttonBackView.layer.cornerRadius = 5;
//    self.buttonBackView.layer.masksToBounds = YES;
//    [self addSubview:_buttonBackView];
  //Content of pickerContainerView
  /*
  //Top bar view
  _pickerTopBarView = [[UIView alloc] initWithFrame:CGRectMake(0.0, 260-44, _pickerContainerView.frame.size.width-0, 44.0)];
  ...
  */
  //Add pickerView
  _pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(0.0, 10, _pickerContainerView.frame.size.width, 205.0)];
  ...
  [_pickerContainerView setTransform:CGAffineTransformMakeTranslation(0.0, CGRectGetHeight(_pickerContainerView.frame))];
    ...
    [_pickerContainerView addSubview:buttonCancel];
    //
    ...
    buttonSure.frame = CGRectMake(0, 244.5, _pickerContainerView.frame.size.width, 41.5);
    [_pickerContainerView addSubview:buttonSure];
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent: (NSInteger)component {
    if (component == 0) {
        ...
        return [[_pickerViewArray objectAtIndex:0]count];
    }else{
        return [[_pickerViewArray objectAtIndex:1]count];
    }
}
...
- (NSString *)pickerView: (UIPickerView *)pickerView
  ...
  if (self.objectToStringConverter == nil){
    ...
    return (self.objectToStringConverter ([[_pickerViewArray objectAtIndex:component]objectAtIndex:row]));
  }
}
...
//        self.onDismissCompletion ([NSString stringWithFormat:@"%@%@",[[_pickerViewArray objectAtIndex:component] objectAtIndex:row],[[_pickerViewArray objectAtIndex:1] objectAtIndex:cIndex]]);
//    }else{
//        NSInteger cIndex = [pickerView selectedRowInComponent:0];
//        self.onDismissCompletion ([NSString stringWithFormat:@"%@%@",[[_pickerViewArray objectAtIndex:0] objectAtIndex:cIndex],[[_pickerViewArray objectAtIndex:1] objectAtIndex:row]]);
//    }
}

- (id)selectedObject {
    NSString *lastString = [NSString stringWithFormat:@"%@%@",[[_pickerViewArray objectAtIndex:0] objectAtIndex:[self.pickerView selectedRowInComponent:0]],[[_pickerViewArray objectAtIndex:1] objectAtIndex:[self.pickerView selectedRowInComponent:1]]];
  return lastString;
}
...
//    [customPickerView addSubview:patternImageView];
    ...
    [pickerViewLabel setFont:[UIFont systemFontOfSize:35]];
  ...
  if (self.objectToStringConverter == nil){
    ...
    [pickerViewLabel setText:(self.objectToStringConverter ([[_pickerViewArray objectAtIndex:component] objectAtIndex:row]))];
  }
...
#pragma mark button
- (void)buttonSureTaped:(UIButton *)sender
{
    [self dismiss];
}

- (void)buttonCancelTaped:(UIButton *)sender
{
    [self cancel];
}

...
#import "BreathLive.h"
static const NSInteger kMaxContainerCapacity = 230;

@interface BreathPointContainer ()
...
@implementation BreathPointContainer
...
{
    static BreathPointContainer *container_ptr = NULL;
...
- (void)addPointAsTranslationChangeform:(CGPoint)point
{
    ...
    if (currentPointsCount < kMaxContainerCapacity) {
        ...
        currentPointsCount ++;
        int workIndex = (int)self.numberOfTranslationElements;
        float last = self.translationPointContainer[workIndex-1].y;
        for (int i = workIndex-1; i>0; --i) {
            self.translationPointContainer[i].y = self.translationPointContainer[i-1].y;
        }
        self.translationPointContainer[0].y = last;
    } else {
        ...
        self.translationPointContainer[0].x = 229;
...
- (void)drawCurve
{
    ...
    CGContextSetStrokeColorWithColor(UIGraphicsGetCurrentContext(), gridColor.CGColor);
    ...
    for (int i = 1; i < self.currentPointsCount; i++) {
        if (self.points[i - 1].x > self.points[i].x) {
            CGContextAddLineToPoint(currentContext, self.points[i-1].x, self.points[i-1].y);
        } else {
            CGContextMoveToPoint(currentContext, self.points[i-1].x, self.points[i-1].y);
        }
    }

...
@interface LiveTurnDataViewController ()

@property (nonatomic, assign) CGFloat viewHeight;
@property (nonatomic, strong) NSArray *dataSource;
@property (nonatomic, strong) TurnLive *heartMoniterView;
...
@implementation LiveTurnDataViewController
...
- (TurnLive *)heartMoniterView
{
    
    if (!_heartMoniterView) {
        _heartMoniterView = [[TurnLive alloc] initWithFrame:CGRectMake(LeftLivePadding, 10, LiveViewWidth, 200)];
...
- (void)getData
{
    NSString *urlString = [NSString stringWithFormat:@"v1/app/SensorDataRealtime?UUID=%@&DataProperty=1&FromDate=&FromTime=",HardWareUUID];
    NSDictionary *header = @{
                             @"AccessToken":@"123456789"
                             };
    GetTurnLiveDataAPI *client = [GetTurnLiveDataAPI shareInstance];
    [client getTurnLiveData:header withDetailUrl:urlString];
...
//                            }
    ...
    [client1 startWithCompletionBlockWithSuccess:^(YTKBaseRequest *request) {
        ...
        //        [KVNProgress dismiss];
        if ([[resposeDic objectForKey:@"ReturnCode"]intValue]==200) {
            int data = [[resposeDic objectForKey:@"BodyMovementTimes"]intValue];
            self.data = data;
...
{
    [[TurnPointContainer sharedContainer] addPointAsTranslationChangeform:[self bubbleTranslationPoint]];
    
    [self.heartMoniterView fireDrawingWithPoints:[[TurnPointContainer sharedContainer] translationPointContainer] pointsCount:[[TurnPointContainer sharedContainer] numberOfTranslationElements]];
...
- (CGPoint)bubbleTranslationPoint
{
    ...
    self.dataSource = self.ceshiArr;
    NSLog(@"体动吐出来的点:%@",NSStringFromCGPoint(targetPointToAdd));
//    self.data = (int)[self.dataSource[dataSourceCounterIndex] integerValue];
//    if ((int)[self.dataSource[dataSourceCounterIndex] integerValue]==1) {
//        self.data +=1;
//    }
//    [self.upTableView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:0]] withRowAnimation:UITableViewRowAnimationNone];

...
@interface TurnroundView ()<UITableViewDataSource,UITableViewDelegate>
{
    BOOL isUp;
}
...
@property (nonatomic,strong) TurnChartView *chartBarView;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.chartTableView.frame = CGRectMake(5, 0, self.frame.size.width-10, self.frame.size.height);
        //
        self.chartBarView = [TurnChartView turnView];
        self.chartBarView.frame = CGRectMake(2, 0, frame.size.width-15, frame.size.height-140-60);
        //设置警告值
        self.chartBarView.horizonLine = 70;
        //设置坐标轴
        if (isUserDefaultTime) {
            self.chartBarView.xValues = @[@"18", @"20", @"22", @"24",@"2",@"4",@"6"];
        }else{
            self.chartBarView.xValues = @[@"18",@"20", @"22", @"24", @"2", @"4", @"6", @"8", @"10", @"12",@"14",@"16",@"18"];
        }
        self.chartBarView.yValues = @[@"2",@"4",@"6",@"8"];
        self.chartBarView.chartColor = selectedThemeIndex==0?DefaultColor:[UIColor whiteColor];
        //
        //
...
{
    self.dataArr = aff;
    self.chartBarView.dataValues = aff;
}

- (void)layoutOutSubViewWithNewData
{
    if (self.chartBarView) {
        [self.chartBarView removeFromSuperview];
        self.chartBarView = nil;
    }
    self.chartBarView = [TurnChartView turnView];
    self.chartBarView.frame = CGRectMake(2, 0, self.frame.size.width-20, self.rowHeight-140-60);
    //设置警告值
    self.chartBarView.horizonLine = 4;
    //设置坐标轴
    if (isUserDefaultTime){
        ...
        if ((startInt<endInt)&&(endInt-startInt>1)&&(endInt - startInt)<12) {
            ...
            _chartBarView.xValues = @[[NSString stringWithFormat:@"%d:00",startInt],[NSString stringWithFormat:@"%d:10",startInt], [NSString stringWithFormat:@"%d:20",startInt],[NSString stringWithFormat:@"%d:30",startInt],[NSString stringWithFormat:@"%d:40",startInt],[NSString stringWithFormat:@"%d:50",startInt],[NSString stringWithFormat:@"%d:00",endInt]];
        ...
        _chartBarView.xValues = @[@"18",@"20", @"22", @"24", @"2", @"4", @"6", @"8", @"10", @"12",@"14",@"16",@"18"];
    }
    self.chartBarView.yValues = @[@"2",@"4",@"6",@"8"];
    self.chartBarView.dataValues = self.dataArr;
    self.chartBarView.chartColor = selectedThemeIndex==0?DefaultColor:[UIColor whiteColor];
...
- (void)layoutOutDefaultSubViewWithNewData
{
    ...
    self.chartBarView = [TurnChartView turnView];
    self.chartBarView.frame = CGRectMake(10, 0, self.frame.size.width-20, self.rowHeight-140-60);
    //设置警告值
    self.chartBarView.horizonLine = 4;
    NSString *startTime = [[NSUserDefaults standardUserDefaults]objectForKey:UserDefaultStartTime];
    NSString *endTime = [[NSUserDefaults standardUserDefaults]objectForKey:UserDefaultEndTime];
    //设置坐标轴
    self.chartBarView.xValues = @[[startTime substringToIndex:2],[endTime substringToIndex:2]];
    self.chartBarView.yValues = @[@"2",@"4",@"6",@"8"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([tableView isEqual:self.chartTableView]) {
        ...
        if (indexPath.row == 0) {
            ...
            cell.cellData = [NSString stringWithFormat:@"%d次/天",[[self.dicToShow objectForKey:@"BodyMovementTimes"] intValue]];
...
//            CGRect rect = self.chartBarView.frame;
//            rect.origin.y = 0;
//            rect.size.height = self.rowHeight - 150-60-44;
//            self.chartBarView.frame = rect;
            [cell addSubview:self.chartBarView];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            cell.backgroundColor = [UIColor clearColor];
            return cell;
        }else{

...
- (instancetype)initWithDelegate:(id<THPinViewDelegate>)delegate
{
    ...
    if (self)
    {
        ...
        [_bottomLeftButton makeConstraints:^(MASConstraintMaker *make) {
            make.centerY.equalTo(_bottomButton.centerY);
            int hei = [THPinNumButton diameter] / 2.0f;
            make.centerX.equalTo(self.left).offset(hei);
        }];
...
{
    UIAlertView *alerView = [[UIAlertView alloc]initWithTitle:@"提示" message:@"是否退出登录并清除锁屏密码" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
    [alerView show];

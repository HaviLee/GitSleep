...
@interface TurnRoundViewController ()<SetScrollDateDelegate,ToggleViewDelegate,SelectedDate>
...
@implementation TurnRoundViewController
...
- (void)createTurnSubView
{
    ...
    self.turnView.breathTimeSwitchButton.toggleDelegate = self;
...
{
    self.turnView.aff = nil;
    NSArray *arr = [dataDic objectForKey:@"SensorData"];
    for (NSDictionary *dic in arr) {
        self.turnView.aff = [dic objectForKey:@"Data"];
...
- (void)getUserAllDaySensorData:(NSString *)fromDate toDate:(NSString *)toDate
{
    ...
    NSString *lastDayString = [NSString stringWithFormat:@"%@",lastDay];
    NSString *newString = [NSString stringWithFormat:@"%@%@%@",[lastDayString substringWithRange:NSMakeRange(0, 4)],[lastDayString substringWithRange:NSMakeRange(5, 2)],[lastDayString substringWithRange:NSMakeRange(8, 2)]];
    NSString *urlString = [NSString stringWithFormat:@"v1/app/SensorDataHistory?UUID=%@&DataProperty=1&FromDate=%@&EndDate=%@&FromTime=18:00&EndTime=18:00",HardWareUUID,newString,toDate];
    NSDictionary *header = @{
                             @"AccessToken":@"123456789"
                             };
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    //保持统一的切换日期
    if (isUserDefaultTime) {
        [self.turnView.breathTimeSwitchButton changeRightImageWithTime:0];
    }else{
        [self.turnView.breathTimeSwitchButton changeLeftImageWithTime:0];
    }
    //和首页保持一致

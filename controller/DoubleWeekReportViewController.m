...
@interface DoubleWeekReportViewController ()

@property (nonatomic, strong) NewWeekReportViewController *leftWeekReport;
@property (nonatomic, strong) NewWeekReportViewController *rightWeekReport;
...
- (void)setPageViewController
{
    ...
    SLPagingViewController *pageViewController = [[SLPagingViewController alloc] initWithNavBarItems:@[navTitleLabel1, navTitleLabel2]
                                                                                    navBarBackground:[UIColor clearColor]
                                                                                               views:@[self.leftWeekReport.view, self.rightWeekReport.view]
                                                                                     showPageControl:YES];
...
- (NewWeekReportViewController *)leftWeekReport
{
    if (!_leftWeekReport) {
        _leftWeekReport = [[NewWeekReportViewController alloc]init];
        
    }
    return _leftWeekReport;
}

- (NewWeekReportViewController *)rightWeekReport
{
    if (!_rightWeekReport) {
        _rightWeekReport = [[NewWeekReportViewController alloc]init];

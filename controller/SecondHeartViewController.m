...
- (void)getUserAllDaySensorData:(NSString *)fromDate toDate:(NSString *)toDate
{
    ...
    if (fromDate) {
        ...
        if (isTodayHourEqualSixteen<18) {
            ...
            urlString = [NSString stringWithFormat:@"v1/app/SensorDataHistory?UUID=%@&UserId=%@&DataProperty=3&FromDate=%@&EndDate=%@&FromTime=18:00&EndTime=18:00",thirdPartyLoginUserId,HardWareUUID,newString,toDate];
        }else{
            self.dateComponentsBase.day = 1;
            ...
            urlString = [NSString stringWithFormat:@"v1/app/SensorDataHistory?UUID=%@&UserId=%@&DataProperty=3&FromDate=%@&EndDate=%@&FromTime=18:00&EndTime=18:00",thirdPartyLoginUserId,HardWareUUID,fromDate,newNextDayString];
        }
        NSDictionary *header = @{

...
//            urlString = [NSString stringWithFormat:@"v1/app/SensorDataHistory?UUID=%@&DataProperty=4&FromDate=%@&EndDate=%@&FromTime=18:00&EndTime=18:00",thirdHardDeviceUUID,fromDate,newNextDayString];
//        }
        NSDictionary *header = @{
...
//            NSString *nextDayString = [NSString stringWithFormat:@"%@",nextDay];
//            NSString *newNextDayString = [NSString stringWithFormat:@"%@%@%@",[nextDayString substringWithRange:NSMakeRange(0, 4)],[nextDayString substringWithRange:NSMakeRange(5, 2)],[nextDayString substringWithRange:NSMakeRange(8, 2)]];
//            urlString = [NSString stringWithFormat:@"v1/app/SleepQuality?UUID=%@&UserId=%@&FromDate=%@&EndDate=%@&FromTime=18:00&EndTime=18:00",thirdHardDeviceUUID,thirdPartyLoginUserId,fromDate,newNextDayString];
//            
//        }
        NSDictionary *header = @{
                                 @"AccessToken":@"123456789"
...
//            [cell addSubview:self.timeSwitchButton];
        ...
        if (indexPath.row==0) {
            cell.rightDataString = [NSString stringWithFormat:@"%d次",[[self.reportData objectForKey:@"AverageRespiratoryRate"] intValue]];
        }else if (indexPath.row==1){
            cell.rightDataString = [NSString stringWithFormat:@"%d次",[[self.reportData objectForKey:@"SlowRespiratoryRateTimes"] intValue]+[[self.reportData objectForKey:@"FastRespiratoryRateTimes"] intValue]];

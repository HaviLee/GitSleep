...
@property (nonatomic, strong) NewQuaterReportViewController *leftQuaterReport;
@property (nonatomic, strong) NewQuaterReportViewController *rightQuaterReport;
...
- (void)setPageViewController
{
    ...
    SLPagingViewController *pageViewController = [[SLPagingViewController alloc] initWithNavBarItems:@[navTitleLabel1, navTitleLabel2]
                                                                                    navBarBackground:[UIColor clearColor]
                                                                                               views:@[self.leftQuaterReport.view, self.rightQuaterReport.view]
                                                                                     showPageControl:YES];
...
- (NewQuaterReportViewController *)leftQuaterReport
{
    if (!_leftQuaterReport) {
        _leftQuaterReport = [[NewQuaterReportViewController alloc]init];
        
    }
    return _leftQuaterReport;
}

- (NewQuaterReportViewController *)rightQuaterReport
{
    if (!_rightQuaterReport) {
        _rightQuaterReport = [[NewQuaterReportViewController alloc]init];
        
    }
    return _rightQuaterReport;
}
